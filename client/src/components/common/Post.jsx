...
const Post = ({ post }) => {
  ...
  const { mutate: deletePost, isPending: isDeleting } = useMutation({
    mutationFn: async () => {
      ...
      try {
        const resp = await fetch(`/api/post/delete/${post._id}`, {
          method: "DELETE",
        });
  ...
  const { mutate: likePost, isPending: isLiking } = useMutation({
    mutationFn: async () => {
      try {
        const resp = await fetch(`/api/post/like/${post._id}`);
        const data = await resp.json();
        if (!resp.ok || data.error) {
  ...
  const { mutate: commentOnPost, isPending: isCommenting } = useMutation({
    mutationFn: async () => {
      try {
        const resp = await fetch(`/api/post/comment`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ text: comment, postId: post._id }),
        });
        const data = await resp.json();
        if (!resp.ok || data.error) {
          toast.error(data.error || "Failed to comment, please try later");
          throw new Error(data.error || "Failed to comment, please try later");
        }
                ...
                <div className="modal-box rounded border border-gray-600">
                  ...
                  <div className="flex flex-col gap-3 max-h-60 overflow-auto">
                    ...
                    {post.comments.map((comment) => (
                      <div
                        key={comment._id}
                        className="flex gap-2 items-start bg-white/10 p-2 rounded-md"
                      >
                        <div className="avatar">
